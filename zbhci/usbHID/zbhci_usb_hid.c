 ...
 *
 * @brief   This is the source file for zbhci_usb_hid
 *
 * @author  Zigbee Group
 * @date    2021
 *
...
static ev_queue_t hidvendor_txPendingQ;


static void hidvendor_send(hidvendor_txBuf_t *pTxBuf){
	if(hidvendor_isAvailable()){
		hidvendor_reportData(pTxBuf);
	}else{
		ev_queue_push(&hidvendor_txPendingQ, (u8*)pTxBuf);
	}
}

static void hidvendor_data_handler(void *arg){
	 /*
...
static void hidvendor_rxCb(u8 *pData){
	TL_SCHEDULE_TASK(hidvendor_data_handler, pData);
	...
	hidvendor_setRxBuf(pBuf);
}

static void hidvendor_txFinishCb(u8 *pData){
    /* Free the TX buffer at first */
    ev_buf_free(pData);

    /* If there is pending data, send it again */
    if(hidvendor_txPendingQ.curNum){
    	hidvendor_txBuf_t *pTxBuf = (hidvendor_txBuf_t *)ev_queue_pop(&hidvendor_txPendingQ);
    	hidvendor_send(pTxBuf);
    }
}

void usb_hid_init(void){
	/* Initialize USB vendor parameters */
	hidvendor_init();

	/* Register callback */
	hidvendor_setCb(hidvendor_rxCb, hidvendor_txFinishCb);
    ...
    ev_queue_init(&hidvendor_txPendingQ, NULL);
}

zbhciTx_e usb_hid_txMsg(u16 u16Type, u16 u16Length, u8 *pu8Data){
	if(u16Length > TX_BUF_LEN){
		return ZBHCI_TX_TOO_LONG;
	}

	hidvendor_txBuf_t *pTxBuf = (hidvendor_txBuf_t *)ev_buf_allocate(sizeof(u16) + u16Length + 7);
	if(!pTxBuf){
